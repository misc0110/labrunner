        ...
        try:
            if proxy:
                ...
                if not self.simulate:
                    proxy_con.connect(hostname=proxy["server"], username=proxy.get("username", ""), password=proxy.get("password", None), pkey=proxy.get("key", None), key_filename=proxy.get("keyfile", None), timeout=int(proxy.get("timeout", 5)))
            ...
            if not self.simulate:
                ...
                ssh.connect(server, username=username, password=password, pkey=key, key_filename=keyfile, sock=transport, timeout=int(self.machine.get("timeout", 5)))
            ...
            if len(self.run):
                ...
                for r in self.run:
                    if not self.simulate:
                        _, ssh_stdout, _ = ssh.exec_command(r)
